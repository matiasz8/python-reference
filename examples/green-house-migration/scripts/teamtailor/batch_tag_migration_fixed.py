#!/usr/bin/env python3
"""
Batch Tag Migration with Progress Tracking (Fixed Version)

This script migrates candidates with structured tags in batches of 10
with detailed progress tracking and visual feedback.

Usage:
    python scripts/teamtailor/batch_tag_migration_fixed.py --dry-run
    python scripts/teamtailor/batch_tag_migration_fixed.py --live --batch-size 10
"""
...
class BatchTagMigrator:
    ...
    def process_batch(
        ...
        for i, candidate in enumerate(candidates, 1):
            ...
            suggested_tags = self.analyze_candidate_data(
                candidate
            )  # Fixed: was candidate_data
...
def print_banner():
    ...
    print("🎯 Batch Tag Migration System (Fixed)")
    print("=" * 60)
...
Examples:
    ...
    python batch_tag_migration_fixed.py --dry-run --limit 50

    # Live migration with custom batch size
    python batch_tag_migration_fixed.py --live --batch-size 5 --limit 100

    # Full migration with progress tracking
    python batch_tag_migration_fixed.py --live --batch-size 10 --limit 1000
        """,
    )
